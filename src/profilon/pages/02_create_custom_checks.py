...
# Databricks SDK is optional (don’t hard-crash if unavailable)
try:
...
with left:
    hero("Checks Viewer", "Preview rows without a SQL Warehouse (serverless job export)")
with right:
...
def _download_workspace_file_bytes(path: str) -> bytes:
    """
    Make wc.files.download work regardless of SDK return shape.
    """
    resp = wc.files.download(file_path=path)
    # Common possibilities: .contents, .data, iterable, or raw bytes
...
def _render_preview(preview_path: str, fmt: str):
    ...
    if preview_path.startswith("/"):
        ...
        # Allow plain relative like "tmp/preview.jsonl" by forcing dbfs: prefix if not provided
